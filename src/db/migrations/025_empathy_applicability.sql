ALTER TABLE call_evaluations
  ALTER COLUMN showed_empathy DROP NOT NULL,
  ADD COLUMN IF NOT EXISTS showed_empathy_applicable boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS showed_empathy_reason text;

UPDATE call_evaluations
SET showed_empathy_applicable=true,
    showed_empathy_reason=coalesce(showed_empathy_reason,'Legacy evaluation; pending applicability reanalysis.')
WHERE showed_empathy_reason IS NULL;

ALTER TABLE call_evaluations DROP CONSTRAINT IF EXISTS call_evaluations_empathy_applicability_check;
ALTER TABLE call_evaluations ADD CONSTRAINT call_evaluations_empathy_applicability_check CHECK (
  (showed_empathy_applicable AND showed_empathy IS NOT NULL)
  OR (NOT showed_empathy_applicable AND showed_empathy IS NULL)
);
...
WHERE EXISTS (
  ...
  JOIN empathy_reanalysis_calls r ON r.id=m.call_recording_id
  WHERE m.recurring_group_id=g.id
);

DELETE FROM analysis_group_members gm
USING empathy_reanalysis_calls r
...
WHERE j.batch_id IN (SELECT DISTINCT batch_id FROM empathy_reanalysis_calls);

UPDATE transcript_segments s SET textual_tone=NULL,updated_at=now()
FROM empathy_reanalysis_calls r
