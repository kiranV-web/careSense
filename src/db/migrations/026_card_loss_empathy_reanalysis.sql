CREATE TEMP TABLE card_loss_empathy_calls ON COMMIT DROP AS
...
WHERE j.batch_id IN (SELECT DISTINCT batch_id FROM card_loss_empathy_calls);

UPDATE transcript_segments s SET textual_tone=NULL,updated_at=now()
FROM card_loss_empathy_calls r
...
SELECT id,'READY',NULL,now(),now() FROM card_loss_empathy_calls
...
WHERE b.id IN (SELECT DISTINCT batch_id FROM card_loss_empathy_calls);
