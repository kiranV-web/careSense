...
const dateParam = { type: 'string', format: 'date-time', description: 'ISO 8601 timestamp' } as const;
...
export const CHAT_TOOLS = [
  {
    ...
    description: 'Count calls grouped by day, week, or month within a date range. Use for "how many calls" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'group_by'],
      properties: { date_from: dateParam, date_to: dateParam, group_by: { type: 'string', enum: ['day', 'week', 'month'] } }
    }
  },
  {
    type: 'function' as const,
    name: 'rank_agents_by_quality',
    description: 'Rank agents by etiquette-rule quality score and resolution rate within a date range. Use for "best/worst agent" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'order', 'min_call_count', 'limit'],
      properties: {
        date_from: dateParam, date_to: dateParam,
  ...
  },
  {
    type: 'function' as const,
    name: 'compare_agents',
    description: 'Get side-by-side quality metrics for a specific set of agents, matched by name (fuzzy) or external ID.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
  ...
  },
  {
    ...
    description: 'Get one specific agent’s etiquette-rule pass/fail rates within a date range. Use for "what should THIS agent improve" questions, as opposed to team-wide questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['agent_name', 'date_from', 'date_to'],
      properties: { agent_name: { type: 'string' }, date_from: dateParam, date_to: dateParam }
    }
  },
  {
    type: 'function' as const,
    name: 'get_team_etiquette_failure_rates',
    description: 'Get the fail rate of each of the 7 call-etiquette rules across the whole team within a date range. Use for "what does everyone miss / what should the team improve" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to'],
      properties: { date_from: dateParam, date_to: dateParam }
    }
  },
  {
    type: 'function' as const,
    name: 'get_issue_category_breakdown',
    description: 'Count calls by issue category within a date range, most common first. Use for "what are customers complaining about" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'limit'],
      properties: { date_from: dateParam, date_to: dateParam, limit: { type: 'integer', minimum: 1, maximum: 50 } }
    }
  },
  {
    type: 'function' as const,
    name: 'get_resolution_breakdown',
    description: 'Count and percentage of calls by resolution status (resolved, unresolved, escalated, dropped, etc.) within a date range. Use for "what % of calls get resolved" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to'],
  ...
  },
  {
    ...
    description: 'Count calls by banking product (e.g. credit cards, loans) within a date range, most common first.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'limit'],
  ...
  },
  {
    ...
    description: 'Get average/median/min/max call duration within a date range, optionally scoped to one agent. Use for "how long are calls" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'agent_name'],
      properties: { date_from: dateParam, date_to: dateParam, agent_name: { type: ['string', 'null'], description: 'Optional agent name to scope to; null for all agents.' } }
    }
  },
  {
    type: 'function' as const,
    name: 'get_flagged_call_counts',
    description: 'Count rude, escalated, recurring, and dropped calls plus urgency-level breakdown within a date range. Use for "how many rude/escalated calls" and urgency questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to'],
  ...
  },
  {
    ...
    description: 'Get the distribution of detected tone/sentiment (e.g. angry, happy, distressed, rude) for either customers or agents within a date range. Use for "how often are customers upset" or "how often are agents rude in tone" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to', 'speaker_role'],
      properties: { date_from: dateParam, date_to: dateParam, speaker_role: { type: 'string', enum: ['AGENT', 'CUSTOMER'] } }
    }
  },
  {
    type: 'function' as const,
    name: 'get_repeat_customers',
    description: 'Find customers with the most calls within a date range. Use for "who calls back the most / frequent callers" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
  ...
  },
  {
    ...
    description: 'Count manager escalation alerts by status (open/in review/closed) and urgency within a date range. Use for "how many alerts are still open" questions.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['date_from', 'date_to'],
  ...
  },
  {
    ...
    description: 'Fetch specific calls (with title, summary, resolution, and quality feedback) matching filters, to cite as evidence for an answer. Always call this before naming or quoting a specific call.',
    strict: true,
    parameters: {
      type: 'object', additionalProperties: false,
      required: ['agent_name', 'resolution_status', 'issue_category', 'etiquette_rule_failed', 'date_from', 'date_to', 'limit'],
      properties: {
...
export async function executeChatTool(name: string, args: Record<string, unknown>, repo: ChatRepository): Promise<unknown> {
  switch (name) {
    case 'get_call_volume':
      return repo.getCallVolume(String(args.date_from), String(args.date_to), args.group_by as 'day' | 'week' | 'month');
    case 'rank_agents_by_quality':
      return repo.rankAgentsByQuality(String(args.date_from), String(args.date_to), args.order as 'best' | 'worst',
        Number(args.min_call_count), Number(args.limit));
    case 'compare_agents':
      return repo.compareAgents(args.agent_names as string[], String(args.date_from), String(args.date_to));
    case 'list_agents':
      return repo.listAgents(Number(args.limit));
    case 'get_agent_etiquette_breakdown':
      return repo.getAgentEtiquetteBreakdown(String(args.agent_name), String(args.date_from), String(args.date_to));
    case 'get_team_etiquette_failure_rates':
      return repo.getTeamEtiquetteFailureRates(String(args.date_from), String(args.date_to));
    case 'get_issue_category_breakdown':
      return repo.getIssueCategoryBreakdown(String(args.date_from), String(args.date_to), Number(args.limit));
    case 'get_resolution_breakdown':
      return repo.getResolutionBreakdown(String(args.date_from), String(args.date_to));
    case 'get_banking_product_breakdown':
      return repo.getBankingProductBreakdown(String(args.date_from), String(args.date_to), Number(args.limit));
    case 'get_call_duration_stats':
      return repo.getCallDurationStats(String(args.date_from), String(args.date_to), (args.agent_name as string | null) ?? undefined);
    case 'get_flagged_call_counts':
      return repo.getFlaggedCallCounts(String(args.date_from), String(args.date_to));
    case 'get_customer_sentiment_breakdown':
      return repo.getCustomerSentimentBreakdown(String(args.date_from), String(args.date_to), args.speaker_role as 'AGENT' | 'CUSTOMER');
    case 'get_repeat_customers':
      return repo.getRepeatCustomers(String(args.date_from), String(args.date_to), Number(args.min_calls), Number(args.limit));
    case 'get_manager_alert_status':
      return repo.getManagerAlertStatus(String(args.date_from), String(args.date_to));
